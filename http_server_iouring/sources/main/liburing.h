...
int io_uring_register_file_alloc_range(struct io_uring *ring, unsigned off,
                                       unsigned len);
...
#define io_uring_for_each_cqe(ring, head, cqe)                                 \
  ...
  for (head = *(ring)->cq.khead;                                               \
       (cqe = (head != io_uring_smp_load_acquire((ring)->cq.ktail)             \
                   ? &(ring)->cq.cqes[io_uring_cqe_index(                      \
                         ring, head, (ring)->cq.ring_mask)]                    \
                   : NULL));                                                   \
       head++)
 ...
 */
static inline void io_uring_prep_splice(struct io_uring_sqe *sqe, int fd_in,
                                        int64_t off_in, int fd_out,
                                        int64_t off_out, unsigned int nbytes,
...
static inline void io_uring_prep_tee(struct io_uring_sqe *sqe, int fd_in,
...
static inline void
io_uring_prep_multishot_accept_direct(struct io_uring_sqe *sqe, int fd,
...
static inline void io_uring_prep_fallocate(struct io_uring_sqe *sqe, int fd,
  ...
  io_uring_prep_rw(IORING_OP_FALLOCATE, sqe, fd, 0, (unsigned int)mode,
                   (__u64)offset);
  sqe->addr = (__u64)len;
}
...
static inline struct cmsghdr *
io_uring_recvmsg_cmsg_nexthdr(struct io_uring_recvmsg_out *o,
                              struct msghdr *msgh, struct cmsghdr *cmsg) {
  unsigned char *end;
...
io_uring_recvmsg_payload_length(struct io_uring_recvmsg_out *o, int buf_len,
                                struct msghdr *msgh) {
...
static inline void io_uring_prep_fgetxattr(struct io_uring_sqe *sqe, int fd,
                                           const char *name, char *value,
                                           size_t len) {
...
static inline void io_uring_prep_fsetxattr(struct io_uring_sqe *sqe, int fd,
                                           const char *name, const char *value,
